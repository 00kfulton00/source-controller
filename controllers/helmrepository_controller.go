...
func (r *HelmRepositoryReconciler) reconcileSource(ctx context.Context, obj *sourcev1.HelmRepository, artifact *sourcev1.Artifact, index *helm.ChartRepository) (ctrl.Result, error) {
	...
	if obj.Spec.SecretRef != nil {
		...
		if err := r.Client.Get(ctx, name, &secret); err != nil {
			conditions.MarkTrue(obj, sourcev1.DownloadFailedCondition, sourcev1.AuthenticationFailedReason, "Failed to get secret '%s': %s", name.String(), err.Error())
			r.Eventf(ctx, obj, events.EventSeverityError, sourcev1.AuthenticationFailedReason, "Failed to get secret '%s': %s", name.String(), err.Error())
		...
		if err != nil {
			conditions.MarkTrue(obj, sourcev1.DownloadFailedCondition, sourcev1.StorageOperationFailedReason, "Failed to create temporary directory for credentials: %s", err.Error())
			r.Eventf(ctx, obj, events.EventSeverityError, sourcev1.AuthenticationFailedReason, "Failed to create temporary directory for credentials: %s", err.Error())
			return ctrl.Result{}, err
		}
		defer os.RemoveAll(tmpDir)
		opts, err := helm.ClientOptionsFromSecret(secret, tmpDir)
		if err != nil {
			conditions.MarkTrue(obj, sourcev1.DownloadFailedCondition, sourcev1.AuthenticationFailedReason, "Failed to configure Helm client with secret data: %s", err)
	...
	if err != nil {
		switch err.(type) {
		case *url.Error:
			conditions.MarkTrue(obj, sourcev1.DownloadFailedCondition, sourcev1.URLInvalidReason, "Invalid Helm repository URL: %s", err.Error())
			return ctrl.Result{}, nil
		default:
			conditions.MarkTrue(obj, sourcev1.DownloadFailedCondition, sourcev1.URLInvalidReason, "Failed to construct Helm client: %s", err.Error())
			return ctrl.Result{}, nil
		}
	}
	if err := newIndex.DownloadIndex(); err != nil {
		conditions.MarkTrue(obj, sourcev1.DownloadFailedCondition, sourcev1.URLInvalidReason, "Failed to download Helm repository index: %s", err.Error())
		return ctrl.Result{}, err
	}
	*index = *newIndex
	...
	*artifact = r.Storage.NewArtifactFor(obj.Kind,
	...
	if !obj.GetArtifact().HasRevision(index.Checksum) {
		conditions.MarkTrue(obj, sourcev1.ArtifactOutdatedCondition, "NewRevision", "Downloaded index revision %s from %s", artifact.Revision, obj.Spec.URL)
...
func (r *HelmRepositoryReconciler) reconcileArtifact(ctx context.Context, obj *sourcev1.HelmRepository, artifact sourcev1.Artifact, index helm.ChartRepository) (ctrl.Result, error) {
	...
	if err != nil {
		ctrl.LoggerFrom(ctx).Error(err, "failed to marshal downloaded index")
		return ctrl.Result{}, err
	}
	if err := r.Storage.AtomicWriteFile(&artifact, bytes.NewReader(b), 0644); err != nil {
		r.Eventf(ctx, obj, events.EventSeverityError,
			sourcev1.StorageOperationFailedReason, "Failed to write Helm repository index to storage: %s", err.Error())
		return ctrl.Result{}, err
	}
